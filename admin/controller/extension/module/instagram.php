    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (isset($this->request->post['instagram_username'])) {
                $username = trim($this->request->post['instagram_username']);
                $get_url = "https://www.instagram.com/".$username."/?__a=1";
                $get_query = file_get_contents($get_url);
                $userdata = json_decode($get_query, true);
                $user_id = $userdata['graphql']['user']['id'];
                $this->request->post['instagram_user_id'] = $user_id;
            } else {
                $this->request->post['instagram_user_id'] = 0;
            }
