...
class ModelToolExportImport extends Model {
	...
	protected function storeManufacturerIntoDatabase( &$manufacturers, $name, &$store_ids, &$available_store_ids ) {
		foreach ($store_ids as $store_id) {
			...
			// проверка на производителя в таблицу oc_manufacturer_description
            if (isset($manufacturers[$name]['manufacturer_id'])) {
                $manufacturer_id=$manufacturers[$name]['manufacturer_id'];
            }
            $sql = "SELECT `manufacturer_id` FROM `".DB_PREFIX."manufacturer_description` WHERE `manufacturer_id`='" . (int)$manufacturer_id."'";
            $result_manuf = $this->db->query( $sql );
            $r=$result_manuf;
            if (empty( $result_manuf->num_rows)){
            	$v=777;
                $sql = "INSERT INTO `".DB_PREFIX."manufacturer_description` SET manufacturer_id='".(int)$manufacturer_id."', language_id= '" .(int)1 . "', name='".$name ."'";
                $this->db->query( $sql );
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		if (count($categories) > 0) {
			...
			foreach ($categories as $category_id) {
				$sql .= ($first) ? "\n" : ",\n";
				if ($first){  $sql .= "($product_id,$category_id,1)"; }else{
                    $sql .= "($product_id,$category_id,0)";
				}
